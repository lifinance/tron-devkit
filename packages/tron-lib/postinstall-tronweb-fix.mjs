...
let files = []
try {
  // Try cwd/node_modules first, then walk up to find hoisted tronweb
  let nodeModulesPath = path.join(process.cwd(), 'node_modules', 'tronweb')
  if (!fs.existsSync(nodeModulesPath)) {
    // Walk up directories to find hoisted node_modules (Bun workspaces hoist to root)
    let dir = process.cwd()
    while (dir !== path.dirname(dir)) {
      dir = path.dirname(dir)
      const candidate = path.join(dir, 'node_modules', 'tronweb')
