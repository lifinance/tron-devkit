...
export function createTronWeb(options: ICreateTronWebOptions): TronWeb {
  ...
  let apiKey: string | undefined = merged[TRON_PRO_API_KEY_HEADER]?.trim()
  if (!apiKey) {
...
/** TronWeb for `tron` / `tronshasta` using env-based RPC config. */
...
/** Read-only / RPC-only (no signing key). */
