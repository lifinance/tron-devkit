...
import { TRON_ZERO_ADDRESS, ZERO_ADDRESS } from './constants'

/**
 * Tron base58 -> EVM 20-byte hex (`0x` + 40 hex chars, lowercase).
 */
 ...
 * EVM zero address maps to Tron's native zero representation.
 */
...
/**
 * Normalize a Tron TVM `address` return value to base58: already-base58,
 * `0x` + 20 bytes, or `41` + 40 hex without `0x`.
 */
...
/**
 * Strict: `0x` + 40 hex or 40 hex only -> Tron base58.
 */
...
/**
 * Base58 `T...`, `0x` + 20 bytes, or raw 20-byte hex -> `0x` + 40 lowercase hex for ABI params.
 */
...
/** Tron base58 form of the TVM zero address. */
